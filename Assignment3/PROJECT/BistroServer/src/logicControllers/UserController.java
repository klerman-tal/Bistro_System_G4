 ...
 * <p>
 * This controller provides a logic layer above {@link User_DB_Controller} and
 * is responsible for:
 * <ul>
 * <li>Logging in subscribers and guests</li>
 * <li>Registering subscribers and restaurant agents (with role-based
 ...
 * <li>Generating new user identifiers based on the current maximum in
 * persistence</li>
 * </ul>
 * </p>
 */
public class UserController {
	 ...
	 * @return a {@link Subscriber} on successful login, or {@code null} if
	 *         validation fails or user not found
	 */
	public Subscriber loginSubscriber(int subscriberId, String username) {
		...
		Subscriber subscriber = userDB.loginSubscriber(subscriberId, username);
	 ...
	 * <p>
	 * At least one of the contact fields (phone, email) must be provided. A new
	 * guest ID is generated using {@link #generateNextUserId()}.
	 * </p>
	 *
	 * @param phone guest phone (may be blank if email is provided)
	 * @param email guest email (may be blank if phone is provided)
	 * @return a {@link User} representing the guest on success, or {@code null} on
	...
	/**
	 * Registers a new subscriber or restaurant agent, subject to role-based
	 * authorization rules.
	 * <p>
	 * Authorization rules implemented:
	 * <ul>
	 * <li>RestaurantAgent can create {@code Subscriber} only</li>
	 * <li>RestaurantManager can create {@code Subscriber} or
	 * {@code RestaurantAgent}</li>
	 ...
	 * @return the new subscriber/agent ID on success, or {@code -1} on
	 *         validation/authorization/DB failure
	 */
	public int registerSubscriber(String username, String firstName, String lastName, String phone, String email,
			Enums.UserRole role, Subscriber performedBy) {

		if (performedBy == null) {
			return -1;
		}

		if (performedBy.getUserRole() == Enums.UserRole.RestaurantAgent && role != Enums.UserRole.Subscriber) {
			return -1;
		}
		...
		if (username == null || username.isBlank() || firstName == null || firstName.isBlank() || lastName == null
				|| lastName.isBlank() || phone == null || phone.isBlank() || email == null || email.isBlank()) {
			return -1;
		}
		...
		try {
			...
			userDB.registerSubscriber(subscriberId, username, firstName, lastName, phone, email, role);
	 ...
	 * <p>
	 * Only a {@code RestaurantManager} is authorized to perform this action. A new
	 * global user ID is generated using {@link #generateNextUserId()}.
	 * </p>
	 *
	 ...
	 * @param performedBy the authenticated subscriber performing the action
	 * @return the new agent ID on success, or {@code -1} on
	 *         validation/authorization/DB failure
	 */
	public int addRestaurantAgent(String username, String firstName, String lastName, String phone, String email,
		...
		try {
			...
			userDB.registerSubscriber(agentId, username, firstName, lastName, phone, email,
	 ...
	 * @return a {@link Subscriber} if found, or {@code null} if the ID is invalid
	 *         or not found
	 */
	 ...
	 * @param subscriberId subscriber identifier
	 * @param performedBy  the authenticated subscriber performing the action
	 * @return the matching {@link Subscriber}, or {@code null} if
	 *         validation/authorization fails or not found
	 */
	 ...
	 * @return the matching {@link Subscriber}, or {@code null} if input is invalid
	 *         or no match exists
	 */
	 ...
	 *         or no match exists
	 */
	public Subscriber recoverSubscriberCode(String username, String phone, String email) {

		if (username == null || username.isBlank() || phone == null || phone.isBlank() || email == null
				|| email.isBlank()) {
			return null;
		}

		Subscriber subscriber = userDB.getSubscriberByUsernamePhoneEmail(username, phone, email);
	...
	public boolean updateSubscriberDetails(int subscriberId, String username, String firstName, String lastName,
		...
		return userDB.updateSubscriberDetails(subscriberId, username, firstName, lastName, phone, email);
	}
