...
/**
 * Handles waiting-list business logic for walk-in customers and table
 * assignment workflows.
 * <p>
 * This controller provides a logic layer above {@link Waiting_DB_Controller}
 * and coordinates with:
 * <ul>
 * <li>{@link RestaurantController} for opening hours checks and time rounding
 * utilities</li>
 * <li>{@link ReservationController} for creating reservations when a waiting
 * entry is seated</li>
 * <li>{@link Notification_DB_Controller} for notifying users about table
 * availability and cancellations</li>
 * </ul>
 * </p>
 * <p>
 ...
 * <li>Assigning a freed table to the next waiting entry (FIFO and seat-capacity
 * based)</li>
 * <li>Confirming arrival within a grace period (15 minutes) to create a
 * reservation</li>
 * <li>Auto-cancelling expired waitings and end-of-day cancellations with
 * notifications</li>
 * </ul>
 * </p>
 */
public class WaitingController {
	 ...
	 * @param restaurantController  restaurant logic controller (opening hours,
	 *                              rounding, etc.)
	 * @param reservationController reservation logic controller used when
	 *                              converting waiting to reservations
	 */
	public WaitingController(Waiting_DB_Controller db, Notification_DB_Controller notificationDB,
			RestaurantServer server, RestaurantController restaurantController,
	...
	/**
	 * Exception thrown when joining the waiting list is blocked due to business
	 * rules (e.g., restaurant is closed or outside opening hours).
	 */
	...
	/**
	 * Attempts to join the waiting list immediately; throws an exception if joining
	 * is blocked.
	 *
	 ...
	 * @throws JoinWaitingBlockedException if the restaurant is closed or joining is
	 *                                     blocked
	 */
	 ...
	 * <p>
	 * The method blocks joining when the restaurant is closed or when the current
	 * time is outside of effective opening hours for today.
	 * </p>
	 * <p>
	 * If the user can be seated immediately, this method attempts to create an
	 * immediate reservation (scenario 1) and marks the waiting entry as seated.
	 * </p>
	 *
	 * @param guestsNumber number of guests
	 * @param user         user requesting to join
	 * @return the created {@link Waiting} entry, or {@code null} if blocked or on
	 *         failure
	 */
	public Waiting joinWaitingListNow(int guestsNumber, User user) {
		...
		try {
			...
			if (now.isBefore(open) || now.isAfter(close)) {
				server.log("JOIN_WAITING blocked: outside business hours. now=" + now + ", open=" + open + ", close="
						+ close);
				return null;
			}
		...
		try {
			int waitingId = db.addToWaitingList(guestsNumber, w.getConfirmationCode(), user.getUserId(),
					user.getUserRole());
		...
		var res = reservationController.createNowReservationFromWaiting(nowSlot, guestsNumber, user,
	 ...
	 * <p>
	 * If a reservation exists with the same confirmation code, the method attempts
	...
	public boolean leaveWaitingList(String confirmationCode) {
		...
		try {
			...
			try {
				reservationController.CancelReservation(code);
	...
	/**
	 * Confirms customer arrival for a waiting entry and attempts to create a
	 * reservation.
	 * <p>
	 ...
	 * <li>The customer arrives within 15 minutes from the table-freed
	 * timestamp</li>
	 * </ul>
	 * </p>
	 *
	 * @param confirmationCode waiting confirmation code
	 * @return {@code true} if the waiting entry was marked seated, {@code false}
	 *         otherwise
	 */
	public boolean confirmArrival(String confirmationCode) {
		...
		try {
			...
			if (w.getWaitingStatus() != WaitingStatus.Waiting)
				return false;
			if (w.getTableFreedTime() == null)
				return false;
			if (w.getTableNumber() == null)
				return false;
			...
			boolean created = reservationController.createReservationFromWaiting(code, start, w.getGuestAmount(), u,
					w.getTableNumber());
	...
	/**
	 * Cancels waiting entries that expired (e.g., no arrival confirmation within
	 * allowed time).
	 *
	...
	public int cancelExpiredWaitings() {
		...
		try {
			...
			for (String code : expiredCodes) {
				...
				server.log("Waiting auto-cancelled (no check-in after 15 min): " + code);
			}
	 ...
	 * <li>Updates the waiting entry with the freed time and assigned table
	 * number</li>
	 * <li>Schedules immediate notifications (SMS and Email)</li>
	 * </ul>
	 * </p>
	 * <p>
	 * This method does not create a reservation directly; the user must confirm
	 * arrival within the grace period to convert waiting to a reservation.
	 * </p>
	 *
	 * @param freedTable table that became available
	 * @return {@code true} if a waiting entry was assigned and updated,
	...
	public boolean handleTableFreed(Table freedTable) {
		...
		try {
			...
			boolean updated = db.setTableFreedForWaiting(next.getConfirmationCode(), now, freedTable.getTableNumber());

			if (!updated)
				return false;

			if (notificationDB != null) {
				String smsEmailBody = "A table is now available. Your waiting code is: " + next.getConfirmationCode()
						+ ". Please arrive within 15 minutes to avoid cancellation.";

				notificationDB.addNotification(new Notification(next.getCreatedByUserId(), Enums.Channel.SMS,
						Enums.NotificationType.TABLE_AVAILABLE, smsEmailBody, now));

				notificationDB.addNotification(new Notification(next.getCreatedByUserId(), Enums.Channel.EMAIL,
						Enums.NotificationType.TABLE_AVAILABLE, smsEmailBody, now));
			}

			server.log("Assigned freed table to waiting. WaitingCode=" + next.getConfirmationCode() + ", Table="
	 ...
	 *
	 * @param confirmationCode waiting confirmation code
	 * @return the matching {@link Waiting}, or {@code null} if not found or on
	 *         error
	 */
	...
	/**
	 * Cancels all active waiting entries for a given date (end-of-day workflow) and
	 * notifies users.
	 *
		...
		try {
			...
			if (count > 0 && notificationDB != null && toCancel != null) {
				...
				for (Waiting w : toCancel) {
					...
					notificationDB.addNotification(new Notification(w.getCreatedByUserId(), Enums.Channel.SMS,
							Enums.NotificationType.TABLE_AVAILABLE, msg, now));

					notificationDB.addNotification(new Notification(w.getCreatedByUserId(), Enums.Channel.EMAIL,
							Enums.NotificationType.TABLE_AVAILABLE, msg, now));
				}
			}
	...
	/**
	 * Retrieves a list of table numbers currently locked by waiting/reservation
	 * allocation logic.
	 *
