...
/**
 * Provides restaurant-level business logic for tables, opening hours, and availability management.
 * <p>
 * This controller acts as a logic layer above {@link Restaurant_DB_Controller} and coordinates:
 * <ul>
 *   <li>Loading, saving, and removing {@link Table} entities (including cache updates)</li>
 *   <li>Managing weekly opening hours and date-specific overrides (special opening hours)</li>
 *   <li>Building and synchronizing the availability grid for the next 30 days</li>
 *   <li>Querying and reserving availability slots for reservation operations</li>
 * </ul>
 * </p>
 * <p>
 * The controller maintains a reference to the {@link Restaurant} singleton as an in-memory cache for
 * relatively stable data (e.g., tables and opening hours).
 * </p>
 */
public class RestaurantController {
     ...
     *
     * @param reservationDB database controller used for reservation-related persistence
     */
     ...
     *
     * @param reservationController reservation logic controller used for reservation-related operations
     */
     ...
     *
     * @param db database controller used for restaurant tables, opening hours, and availability grid persistence
     */
     ...
     *
     * @param specialDB database controller used for date-specific opening hours overrides
     */
    ...
    /**
     * Saves a new table or updates an existing table, both in the cache and in the database.
     * <p>
     * After saving, the availability grid schema is ensured to match the current sorted table list.
     * </p>
     *
    ...
    /**
     * Removes a table from the system and updates the availability grid schema accordingly.
     * <p>
     * If a {@link ReservationController} is available, upcoming reservations for the deleted table
     * are first relocated or cancelled to ensure consistency before the deletion occurs.
     * </p>
     *
     * @param tableNumber table number to remove
     * @return {@code true} if the table was deleted successfully, {@code false} otherwise
     * @throws SQLException if a database operation fails
     */
    ...
    /**
     * Finds one available table for a given slot while excluding a specific table number.
     *
    ...
    /**
     * Loads weekly opening hours from the database into the {@link Restaurant} cache.
     *
     * @throws SQLException if the database query fails
     */
    ...
    /**
     * Retrieves weekly opening hours from the cache, refreshing it from the database first.
     *
    ...
    /**
     * Finds weekly opening hours for the given date based on the day of week (English full name).
     *
     ...
     * </p>
     * <p>
     * When the date is marked as closed in special opening hours, the returned {@link OpeningHouers}
     * contains {@code null} open/close times.
     * </p>
     *
     * @param date target date
     * @return effective {@link OpeningHouers} for that date, or {@code null} if no data exists
     */
     ...
     * <p>
     * The method ensures table schema is aligned, deletes past slots, and then initializes
     * new slots using the effective opening hours for each date.
     * </p>
     *
    ...
    /**
     * Rebuilds availability grid slots for a specific date based on effective opening hours.
     * <p>
     * Existing grid slots for the date are deleted first, then the grid is re-initialized.
     * If the effective hours indicate the date is closed, the grid remains empty for that date.
     * </p>
     *
    ...
    /**
     * Finds one available table for a given slot while excluding a list of table numbers.
     *
     * @param slot                 reservation slot date-time
     * @param peopleCount          number of guests
     * @param excludedTableNumbers list of table numbers to exclude (may be {@code null})
     * @return an available {@link Table} or {@code null} if none is available
     * @throws Exception if database access or availability checks fail
     */
    ...
    /**
     * Returns a mapping of available reservation slots to one available table per slot for a given date.
     *
     * @param date        target date
     * @param peopleCount number of guests
     ...
     * @return {@code true} if the slot was reserved successfully, {@code false} otherwise
     * @throws Exception if slot is invalid, table does not exist, or a database operation fails
     */
     ...
     * @return {@code true} if the slot was marked available successfully, {@code false} otherwise
    ...
    /**
     * Checks whether a specific table is free for a full two-hour window starting at {@code start}.
     * <p>
     ...
     * @return {@code true} if the table is free for all 4 slots, {@code false} otherwise
     ...
     * Returns an empty string if the input is {@code null} or does not match supported formats.
     * </p>
     *
     * @param t time string to normalize
     * @return normalized time string in {@code HH:MM}, or empty string on invalid input
     */
    ...
    /**
     * Returns all dates in the next 30 days (including today) that match the given weekday.
     * <p>
     * The input is expected to be a weekday name supported by {@link DayOfWeek#valueOf(String)}
     * after uppercasing (e.g., {@code "Thursday"}).
     * </p>
     *
