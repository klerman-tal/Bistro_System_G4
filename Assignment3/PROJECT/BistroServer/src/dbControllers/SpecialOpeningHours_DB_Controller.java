...
/**
 * Provides persistence operations for special (date-specific) opening hours
 * overrides.
 * <p>
 * This controller manages the {@code special_opening_hours} table, which can
 * override regular weekly opening hours for a specific date. A special date can
 * either define explicit opening/closing times or mark the restaurant as
 * closed.
 * </p>
 * <p>
 * Typical use-cases:
...
public class SpecialOpeningHours_DB_Controller {
	...
	/**
	 * Constructs a SpecialOpeningHours_DB_Controller with the given JDBC
	 * connection.
	 *
	...
	/**
	 * Inserts or updates a special opening hours override (UPSERT) for a specific
	 * date.
	 * <p>
	 * Uses {@code ON DUPLICATE KEY UPDATE} so the operation is idempotent for the
	 * same {@code special_date}.
	 * </p>
	 *
	 * @param special special opening hours entity containing date, times, and
	 *                closed flag
	 * @return {@code true} if the statement affected rows, otherwise {@code false}
	 * @throws SQLException if a database error occurs during insert/update
	 *                      execution
	 */
	 ...
	 * @return a {@link SpecialOpeningHours} instance if found; otherwise
	 *         {@code null}
	...
	public SpecialOpeningHours getSpecialHoursByDate(LocalDate date) throws SQLException {
		...
		try (PreparedStatement ps = conn.prepareStatement(sql)) {
			...
			try (ResultSet rs = ps.executeQuery()) {
				...
				return new SpecialOpeningHours(rs.getDate("special_date").toLocalDate(), rs.getTime("open_time"),
						rs.getTime("close_time"), rs.getBoolean("is_closed"));
			}
		}
	}
	...
	public ArrayList<SpecialOpeningHours> getAllSpecialOpeningHours() throws SQLException {
		...
		try (PreparedStatement ps = conn.prepareStatement(sql); ResultSet rs = ps.executeQuery()) {
			...
			while (rs.next()) {
				list.add(new SpecialOpeningHours(rs.getDate("special_date").toLocalDate(), rs.getTime("open_time"),
						rs.getTime("close_time"), rs.getBoolean("is_closed")));
			}
		}
		return list;
