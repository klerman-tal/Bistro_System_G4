 ...
 * subscriber, delegates the deletion logic to the {@link UserController}, and
...
public class DeleteSubscriberHandler implements RequestHandler {
	 ...
	 * <p>
	 * The method checks the session user for authorization, performs the deletion
	 * using the user controller, and sends a response indicating whether the
	 * operation succeeded.
	 * </p>
	 */
	@Override
	public void handle(RequestDTO request, ConnectionToClient client) {
		...
		try {
			...
			DeleteSubscriberDTO data = (DeleteSubscriberDTO) request.getData();

			boolean deleted = userController.deleteSubscriber(data.getSubscriberId(), performedBy);

			client.sendToClient(
					new ResponseDTO(deleted, deleted ? "Subscriber deleted successfully" : "Delete failed", null));
