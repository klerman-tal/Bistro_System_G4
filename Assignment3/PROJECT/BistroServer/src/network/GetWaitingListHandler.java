 ...
 * list.
 * <p>
 * This handler fetches all currently active waiting entries from the
 * {@link WaitingController} and returns them to the client. It is typically
 * used by restaurant staff to monitor the waiting list.
 * </p>
 */
public class GetWaitingListHandler implements RequestHandler {
	 ...
	 * <p>
	 * The method loads all active waiting entries and sends them back to the client
	 * as part of the response. In case of an error, a failure response is returned.
	 * </p>
	 */
	@Override
	public void handle(RequestDTO request, ConnectionToClient client) {
		...
		try {
			...
			response = new ResponseDTO(true, "Waiting list retrieved successfully", waitingList);
		} catch (Exception e) {
			response = new ResponseDTO(false, "Failed to retrieve waiting list: " + e.getMessage(), null);
		}
