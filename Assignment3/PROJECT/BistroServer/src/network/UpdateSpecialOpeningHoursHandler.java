 ...
 * <p>
 * This handler manages exceptional opening rules (such as holidays or special
 * events) that override the regular weekly opening hours. It ensures
 * consistency between:
 * <ul>
...
public class UpdateSpecialOpeningHoursHandler implements RequestHandler {
	 ...
	 *                              management
	 */
	public UpdateSpecialOpeningHoursHandler(SpecialOpeningHours_DB_Controller specialDb,
			RestaurantController restaurantController, ReservationController reservationController) {
	...
	/**
	 * Handles a request to create or update special opening hours for a specific
	 * date.
	 * <p>
	 ...
	 * <li>Returns the updated list of special opening hours to refresh the client
	 * UI</li>
	 * </ol>
	 * </p>
	 */
	...
	public void handle(RequestDTO request, ConnectionToClient client) {
		...
		try {
			...
			SpecialOpeningHours entity = new SpecialOpeningHours(dto.getSpecialDate(), dto.getOpenTime(),
					dto.getCloseTime(), dto.isClosed());
			...
			int cancelled = reservationController.cancelReservationsDueToOpeningHoursChange(entity.getSpecialDate(),
					entity.getOpenTime() == null ? null : entity.getOpenTime().toLocalTime(),
					entity.getCloseTime() == null ? null : entity.getCloseTime().toLocalTime(), entity.isClosed());

			// 4) Return updated special opening hours list so GUI refreshes immediately
			client.sendToClient(new ResponseDTO(true, "Special hours updated. Cancelled reservations: " + cancelled,
