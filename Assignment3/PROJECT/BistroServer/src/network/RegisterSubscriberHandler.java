 ...
 * {@link UserController}. It is typically used by restaurant agents or
 * managers through management interfaces.
 * </p>
 */
public class RegisterSubscriberHandler implements RequestHandler {
     ...
     * <ol>
     *   <li>Validates that the request data is of type {@link RegisterSubscriberDTO}</li>
     *   <li>Verifies that the current session user is authorized to register subscribers</li>
    ...
    public void handle(RequestDTO request, ConnectionToClient client) {
        ...
        if (createdId == -1) {
            send(client, new ResponseDTO(
                    false,
                    "Subscriber was not created: permission denied or invalid data",
                    null
            ));
            return;
        }
     ...
     * <p>
     * This helper method centralizes error handling for network transmission
     * and prevents duplicated try/catch blocks inside the handler logic.
     * </p>
     */
