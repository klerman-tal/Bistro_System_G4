 ...
 * This handler:
 * <ul>
 * <li>Validates guest identification details (phone, email, reservation
 * date/time)</li>
 * <li>Delegates code recovery to {@link ReservationController}</li>
 * <li>Creates simulated EMAIL and SMS notifications (stored and marked as sent
 * in DB)</li>
 * <li>Sends a safe in-app popup message (without sensitive data) via
 * {@link NotificationDTO}</li>
...
public class RecoverGuestConfirmationCodeHandler implements RequestHandler {
	...
	/**
	 * Constructs a handler with required dependencies for recovery and
	 * notifications.
	 *
	 * @param rc             controller used for reservation lookup and code
	 *                       recovery
	 * @param uc             user controller (dependency provided for consistency /
	 *                       future use)
	 * @param notificationDB database controller used to persist notification audit
	 *                       records
	 * @param logger         optional logger callback for server-side
	 *                       tracing/debugging
	 */
	public RecoverGuestConfirmationCodeHandler(ReservationController rc, UserController uc,
			Notification_DB_Controller notificationDB, Consumer<String> logger) {
	 ...
	 * <li>Sends an in-app popup notification (safe display text, SMS body includes
	 * the code)</li>
	 * <li>Sends a final response to the client with the recovered code</li>
	 * </ol>
	 * </p>
	 *
	 * @param request the incoming request containing
	 *                {@link RecoverGuestConfirmationCodeDTO}
	...
	public void handle(RequestDTO request, ConnectionToClient client) throws Exception {
		...
		/**
		 * Step 2: Attempt to recover the reservation confirmation code based on guest
		 * identifying details and reservation date/time.
		 */
		String code = reservationController.recoverGuestConfirmationCode(data.getPhone(), data.getEmail(),
				data.getReservationDateTime());

		if (code == null) {
			client.sendToClient(new ResponseDTO(false, "Reservation not found. Please check details.", null));
			return;
		}

		/**
		 * Step 3: Create audit records for simulated SMS/Email notifications. These
		 * records are stored in DB and marked as sent for traceability.
		 */
		...
		// Safe display text (no code), while channel content contains the recovered
		// code.
		 ...
		 */
		int emailId = notificationDB.addNotification(new Notification(userIdForAudit, Enums.Channel.EMAIL,
				Enums.NotificationType.RESEND_CONFIRMATION_CODE, emailBody, now));
		if (emailId != -1) {
		 ...
		 */
		int smsId = notificationDB.addNotification(new Notification(userIdForAudit, Enums.Channel.SMS,
				Enums.NotificationType.RESEND_CONFIRMATION_CODE, smsBody, now));
		if (smsId != -1) {
			notificationDB.markAsSent(smsId, now);
		}

		/**
		 * Step 6: Send in-app popup notification (safe UI message without sensitive
		 * data). The SMS simulation content contains the actual code.
		 */
		client.sendToClient(new NotificationDTO(NotificationDTO.Type.INFO, "SMS", displayMessage, smsBody));

		/**
		 * Step 7: Send a final response message to complete the request. A silent
		 * response avoids triggering an additional popup on the client.
		 */
