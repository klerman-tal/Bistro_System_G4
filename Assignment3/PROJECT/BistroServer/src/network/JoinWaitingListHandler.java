...
/**
 * Server-side request handler responsible for joining the restaurant waiting
 * list.
 * <p>
 * This handler processes waiting list join requests, validates the input data,
 * constructs a minimal user context, and delegates the join logic to the
 * {@link WaitingController}. The response indicates whether the user was seated
 * immediately or added to the waiting list.
 * </p>
 */
public class JoinWaitingListHandler implements RequestHandler {
	 ...
	 * <p>
	 * The method validates the number of guests, builds a minimal user object from
	 * the request data, attempts to join the waiting list, and returns the
	 * resulting waiting status to the client.
	 * </p>
	 */
	@Override
	public void handle(RequestDTO request, ConnectionToClient client) throws Exception {
		...
		if (w.getWaitingStatus() == WaitingStatus.Seated) {
			client.sendToClient(new ResponseDTO(true, "Table available now. Please go to the table.", w));
		} else {
			client.sendToClient(new ResponseDTO(true, "Joined waiting list. Please wait for a table.", w));
		}
	}
}
