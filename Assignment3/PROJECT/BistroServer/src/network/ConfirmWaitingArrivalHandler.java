...
/**
 * Server-side request handler responsible for confirming a waiting list
 * arrival.
 * <p>
 * This handler validates the waiting confirmation code, delegates the arrival
 * confirmation logic to the {@link WaitingController}, and returns the updated
...
public class ConfirmWaitingArrivalHandler implements RequestHandler {
	 ...
	 * <p>
	 * The method validates the confirmation code, confirms the user's arrival in
	 * the waiting list, and returns the updated waiting state if successful.
	 * </p>
	 */
	@Override
	public void handle(RequestDTO request, ConnectionToClient client) throws Exception {
		...
		if (!ok) {
			client.sendToClient(
					new ResponseDTO(false, "Arrival confirm failed (expired / not ready / not found)", null));
			return;
		}
