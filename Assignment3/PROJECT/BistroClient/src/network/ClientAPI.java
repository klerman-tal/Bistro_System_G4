...
/**
 * Provides a client-side API for sending typed requests to the server via {@link ChatClient}.
 *
 * <p>This class is a thin wrapper around the networking layer that:
 * builds the appropriate DTO payloads, validates basic input constraints, wraps them into
 * {@link RequestDTO} objects with the correct {@link Commands}, and sends them to the server.</p>
 *
 * <p>Validation in this class focuses on preventing clearly invalid requests from being sent
 * (e.g., missing confirmation codes, null dates/times, or invalid reservation time windows).</p>
 */
public class ClientAPI {
    ...
    /**
     * Creates a new {@code ClientAPI} that sends requests using the provided {@link ChatClient}.
     *
     ...
     *
     * <p>Rules enforced by this method:
     * the reservation must be at least 1 hour in advance and no more than 1 month in advance.</p>
     *
     ...
     * @throws IllegalArgumentException if input data is missing/invalid or violates time rules
    ...
    /**
     * Requests the special opening hours configuration (date-based overrides) from the server.
     *
     * @throws IOException if sending the request to the server fails
     */
     ...
     *
     * <p>If {@code isClosed} is {@code true}, the server may treat {@code openTime}/{@code closeTime}
     * as irrelevant depending on business rules.</p>
     *
     * @param date      the date to update
     * @param openTime  opening time for the given date (may be null depending on server rules)
     * @param closeTime closing time for the given date (may be null depending on server rules)
     ...
     * @param email                guest email address (optional if phone is provided)
     * @param reservationDateTime  the reservation date and time
     * @throws IllegalArgumentException if both phone and email are missing/blank, or if date/time is missing
     * @throws IOException              if sending the request to the server fails
     */
     ...
     * @throws IllegalArgumentException if {@code guests <= 0} or {@code user} is {@code null}
     * @throws IOException              if sending the request to the server fails
     */
     ...
     *
     * <p>If {@code guests <= 0}, this method defaults to 1 guest.</p>
     *
    ...
    /**
     * Requests the current number (or list) of diners from the server (depending on server implementation).
     *
     * @throws IOException if sending the request to the server fails
     */
