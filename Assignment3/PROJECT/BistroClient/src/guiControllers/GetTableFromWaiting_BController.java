...
/**
 * JavaFX controller for confirming arrival from the waiting list and receiving
 * a table assignment.
 *
 * <p>
 * This screen allows a user to enter a waiting-list confirmation code and
 * confirm arrival. For non-random users, it can also display a table of the
 * user's active waiting entries and allows selecting a row to prefill the
 * confirmation code.
 * </p>
 *
 * <p>
 * The controller communicates with the server through {@link ClientAPI} and
 * processes asynchronous responses via {@link ClientResponseHandler}.
 * </p>
 */
public class GetTableFromWaiting_BController implements ClientResponseHandler {
	...
	@FXML
	private VBox boxMyActive;
	@FXML
	private TableView<Waiting> tblMyActive;
	@FXML
	private TableColumn<Waiting, LocalDate> colDate;
	@FXML
	private TableColumn<Waiting, LocalTime> colTime;
	@FXML
	 ...
	 * this controller as the active response handler, and initializes the "My
	 * Active Waitings" table.
	 *
	 ...
	 *
	 * <p>
	 * Configures column value factories and attaches a listener to prefill the
	 * confirmation code input field when a row is selected.
	 * </p>
	 */
	private void initMyActiveTable() {
		...
		colCode.setCellValueFactory(
				cd -> new SimpleStringProperty(cd.getValue() == null ? "" : cd.getValue().getConfirmationCode()));
	...
	/**
	 * Loads active waiting entries for the current user if the role allows it and
	 * toggles table visibility.
	 *
	 * <p>
	 * The active waitings table is hidden for {@link UserRole#RandomClient} and
	 * shown for other roles.
	 * </p>
	 */
	 ...
	 *
	 * <p>
	 * Validates that the confirmation code is a 6-digit number and sends a
	 * confirm-arrival request to the server.
	 * </p>
	 */
	@FXML
	 ...
	 * <li>Confirm-arrival results (to show table assignment and success
	 * message)</li>
	 * </ul>
	 *
	...
	/**
	 * Navigates back to the table-choice screen and clears the active response
	 * handler.
	 */
	@FXML
	...
	/**
	 * Loads the requested FXML and swaps the current scene root to navigate between
	 * screens.
	 *
	 * <p>
	 * If the target controller defines {@code setClientActions(ClientActions)}
	 * and/or {@code setClient(User, ChatClient)}, these will be invoked
	 * reflectively to preserve session context.
	 * </p>
	 *
