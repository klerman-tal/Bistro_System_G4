 ...
 *
 * <p>This screen serves as an entry point to the different reports (e.g., time report and subscribers report).
 * It also presents a short summary describing when monthly reports become available, based on the
 * "last full month" rule used across report screens.</p>
 *
 * <p>Navigation is performed by swapping the current scene root to keep the same window instance
 * and preserve the maximized state.</p>
 */
public class ReportsMenuController {
    ...
    /**
     * Loads an FXML screen, injects the current session context into the target controller
     * (when supported), and navigates by swapping the current scene root.
     *
