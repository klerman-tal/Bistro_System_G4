 ...
 * <p>
 * This screen is opened after a successful login and serves as the navigation
 * hub. It also supports a staff "Act as" mechanism that allows Restaurant
 * Agents/Managers to perform actions on behalf of another user (registered user
 * by ID) or a newly created guest (by phone).
 * </p>
 *
 * <h3>Main responsibilities</h3>
 * <ul>
 * <li>Receives and stores the current session context (logged-in {@link User} +
 * {@link ChatClient}).</li>
 * <li>Controls visibility of menu buttons according to the logged-in user's
 * role.</li>
 ...
 * </ul>
 * </li>
 * <li>Performs navigation by reusing the same {@link Scene} and replacing only
 * the root.</li>
 * <li>Implements {@link ClientResponseHandler} to receive async responses (find
 * user / create guest).</li>
 * </ul>
 */
public class Menu_BController implements ClientResponseHandler {
	...
	@FXML
	private BorderPane actAsBox;
	@FXML
	private TextField txtIdForRegistered;
	@FXML
	private TextField txtPhoneForNewGuest;
	@FXML
	private Button btnOk;
	@FXML
	private Label lblActingUser;
	...
	/**
	 * Optional actions interface (injected from outside), passed to controllers
	 * that support it.
	 */
	 ...
	 * <li>Updates {@link ClientSession} (logged-in user, acting user, chat
	 * client)</li>
	 * <li>Registers this controller as the current
	 * {@link ClientResponseHandler}</li>
	...
	public void setClient(User user, ChatClient chatClient) {
		...
		boolean isStaff = user != null && (user.getUserRole() == Enums.UserRole.RestaurantAgent
		...
		boolean showPersonal = user != null && user.getUserRole() != Enums.UserRole.RandomClient;
	...
	/**
	 * Injects {@link ClientActions} (optional). Some screens may expect this
	 * interface for actions beyond direct server calls.
	 *
	...
	/**
	 * Resets the acting user to the logged-in user. This is useful after finishing
	 * actions performed on behalf of someone else.
	 */
	@FXML
	...
	/**
	 * Logs out and navigates back to the login screen. Clears the session users in
	 * {@link ClientSession}.
	 */
	@FXML
	 ...
	 * <li>{@link User} + {@link ChatClient} via a reflective setClient(User,
	 * ChatClient) if present</li>
	 * </ul>
	 * </p>
	 *
	 * @param fxmlName         FXML file name (located under /gui/)
	 * @param title            window title suffix
	 * @param managementScreen if true, pass logged-in user; otherwise pass acting
	 *                         user
	 */
	private void openWindow(String fxmlName, String title, boolean managementScreen) {
		try {
			...
			User userToPass = managementScreen ? ClientSession.getLoggedInUser() : ClientSession.getActingUser();
	 ...
	 * <p>
	 * On success with a {@link User} payload, updates {@link ClientSession}'s
	 * acting user.
	 * </p>
	 *
	...
	/**
	 * Called when the connection is closed while this controller is active. Shows a
	 * message to the user.
	 */
	@Override
	...
	private void updateActingUserLabel() {
		...
		lblActingUser.setText(acting == null ? "Acting User: —"
	...
	private void clearOkFields() {
		if (txtIdForRegistered != null)
			txtIdForRegistered.clear();
		if (txtPhoneForNewGuest != null)
			txtPhoneForNewGuest.clear();
	}
