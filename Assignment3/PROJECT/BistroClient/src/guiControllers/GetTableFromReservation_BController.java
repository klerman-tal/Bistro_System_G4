...
/**
 * JavaFX controller for checking in to an existing reservation and receiving a
 * table assignment.
 *
 * <p>
 * This screen allows a user to enter a reservation confirmation code and send a
 * check-in request. For non-random users, it can also display a table of the
 * user's active reservations, allowing quick selection of the confirmation
 * code.
 * </p>
 *
 * <p>
...
public class GetTableFromReservation_BController implements ClientResponseHandler {
	...
	@FXML
	private TableView<Reservation> tblMyActive;
	@FXML
	private TableColumn<Reservation, LocalDate> colDate;
	@FXML
	private TableColumn<Reservation, LocalTime> colTime;
	@FXML
	 ...
	 *
	 * <p>
	 * The active reservations table is hidden for {@link UserRole#RandomClient} and
	 ...
	 *
	 * <p>
	 * Validates that the confirmation code is a 6-digit number and sends a check-in
	...
	public void handleResponse(ResponseDTO response) {
		Platform.runLater(() -> {
			...
			if (res != null) {
				...
				if (res.isShouldWait()) {
					showInfo(res.getMessage() != null ? res.getMessage()
