 ...
 *
 * <p>
 * This controller collects a reservation confirmation code and a cancellation
 * reason, builds a {@link CancelReservationDTO}, and sends a
 * {@link Commands#CANCEL_RESERVATION} request to the server via
 * {@link ChatClient}. It also implements {@link ClientResponseHandler} to
 * display server responses on the UI thread.
 * </p>
 */
public class CancelReservation_BController implements ClientResponseHandler {
	...
	@FXML
	private ComboBox<String> cmbCancelReason;
	@FXML
	private TextArea txtOtherReason;
	@FXML
	 ...
	 *
	 * <p>
	 * Populates the cancellation reason combo-box with predefined options.
	 * </p>
	 */
	@FXML
	public void initialize() {
		cmbCancelReason.getItems().addAll("Changed my plans", "Found another restaurant", "Running late",
	...
	/**
	 * Sets the current user and client instance for this screen and registers this
	 * controller as the active response handler.
	 *
	...
	/**
	 * Sets the current user and client instance and optionally pre-fills the
	 * confirmation code (e.g., when navigating from a table selection).
	 *
	 * @param user             the current logged-in user
	 * @param chatClient       the network client used to send requests
	 ...
	 *
	 * <p>
	 * Validates input, builds a cancellation DTO, and sends the request to the
	 * server.
	 * </p>
	 */
	@FXML
	private void onCancelReservationClicked() {
		...
		CancelReservationDTO data = new CancelReservationDTO(code.trim(), finalReason);
