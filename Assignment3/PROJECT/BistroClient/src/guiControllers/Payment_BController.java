...
/**
 * JavaFX controller that allows a user to load a receipt by confirmation code
 ...
 * </p>
 * <ol>
 * <li>Load a receipt by entering a confirmation code and requesting the amount
 * from the server.</li>
 * <li>Pay the loaded receipt using either credit card details or cash.</li>
 * </ol>
 *
 * <p>
 * Server communication is performed through {@link ClientAPI}. This controller
 * implements {@link ClientResponseHandler} and uses a small internal state
 * machine ({@link PendingAction}) to determine whether the current response
 * belongs to a "load" request or a "pay" request.
 * </p>
 *
 * <p>
 * The controller stores and restores the previous response handler so it does
 * not break other screens when navigating away.
 * </p>
 */
public class Payment_BController implements ClientResponseHandler {
	...
	@FXML
	private TextField txtLast4Digits;
	@FXML
	...
	/**
	 * Initializes UI defaults, input constraints, and listeners. This method is
	 * invoked automatically by JavaFX after FXML injection.
	 */
	@FXML
	...
	/**
	 * Adds a real-time input sanitizer/limiter to a {@link TextField}. The limiter
	 * enforces numeric input only, optionally allowing "/" for MM/YY fields, and
	 * truncates input to the given maximum length.
	 *
	...
	/**
	 * Updates the UI state based on whether a receipt is currently loaded. When not
	 * loaded, payment is disabled and cached receipt data is cleared.
	 *
	 * @param loaded {@code true} to enable payment actions; {@code false} to reset
	 *               state
	 */
	...
	/**
	 * Injects the session context and sets up the API. Saves the previously active
	 * response handler and replaces it with this controller.
	 *
	 * @param user       the current logged-in user
	 * @param chatClient the active client connection used to communicate with the
	 *                   server
	 */
	...
	/**
	 * Loads a receipt from the server using the confirmation code entered by the
	 * user. Disables payment until a receipt is successfully loaded.
	 */
	@FXML
	...
	/**
	 * Validates user input according to the selected payment type and sends a
	 * payment request. Requires a previously loaded receipt.
	 */
	@FXML
	private void onPayClicked() {
		...
		if (rbCreditCard.isSelected()) {
			...
			sendPayment(new PayReceiptDTO(loadedCode, Enums.TypeOfPayment.CreditCard, Integer.parseInt(last4), expiry,
	...
	/**
	 * Validates that the provided MM/YY string is well-formed and represents the
	 * current month or a future month.
	 *
	 * @param dateStr expiry date in the format MM/YY
	 * @return {@code true} if the date is valid and not in the past; otherwise
	 *         {@code false}
	 */
	private boolean isValidFutureDate(String dateStr) {
		if (!dateStr.matches("(0[1-9]|1[0-2])/[0-9]{2}"))
			return false;
		try {
	...
	/**
	 * Sends the payment request to the server and marks the pending action as
	 * {@link PendingAction#PAY}.
	 *
	...
	/**
	 * Routes the server response to the appropriate handler based on
	 * {@link #pending}.
	 *
	...
	/**
	 * Handles the response of a "load receipt" request. On success, caches the
	 * receipt and enables payment.
	 *
	 * @param response server response containing the loaded {@link Receipt} on
	 *                 success
	 */
	...
	/**
	 * Handles the response of a payment request. On success, clears the loaded
	 * receipt state.
	 *
	...
	/**
	 * Restores the previous response handler (if available) and navigates back to
	 * the "My Visit" menu.
	 */
	@FXML
	 ...
	 * Attempts to inject (User, ChatClient) to the destination controller via
	 * reflection.
	 *
