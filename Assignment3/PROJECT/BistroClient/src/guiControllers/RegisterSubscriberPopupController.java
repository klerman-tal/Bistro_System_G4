...
/**
 * JavaFX popup controller used to create a new subscriber (or staff user) from
 * the management UI.
 *
 * <p>
 * The popup collects basic user details (name, username, phone, email) and a
 * role that is restricted by the role of the user who opened the popup (manager
 * vs. agent). Input is validated locally before sending the request to the
 * server using {@link ClientAPI}.
 * </p>
 *
 * <p>
 * On successful validation, the controller sends the registration request and
...
public class RegisterSubscriberPopupController {
	...
	@FXML
	private ComboBox<Enums.UserRole> roleCombo;
	@FXML
	...
	/**
	 * Initializes the popup controls and installs real-time input constraints. This
	 * method is called automatically by JavaFX after FXML injection.
	 */
	@FXML
	...
	/**
	 * Configures the available roles in the role dropdown according to the role of
	 * the user who opened this popup.
	 *
	...
	public void setPerformedByRole(Enums.UserRole performedByRole) {
		...
		if (performedByRole == Enums.UserRole.RestaurantManager) {
			roleCombo.getItems().setAll(Enums.UserRole.Subscriber, Enums.UserRole.RestaurantAgent,
					Enums.UserRole.RestaurantManager);
		} else if (performedByRole == Enums.UserRole.RestaurantAgent) {
			roleCombo.getItems().setAll(Enums.UserRole.Subscriber);
		}
	...
	/**
	 * Validates the form fields and sends a "register subscriber" request to the
	 * server. Highlights invalid fields and shows a message if validation fails.
	 */
	@FXML
	private void handleCreate() {
		...
		try {
			clientAPI.registerSubscriber(username, firstName, lastName, phone, email, role);
	 ...
	 * @return {@code true} if the email is considered valid; otherwise
	 *         {@code false}
	 */
	...
	/**
	 * Closes the popup window using one of the injected controls to locate the
	 * stage.
	 */
